    ...
    <div class="sidebar-limits">
        <div class="sidebar-section">
            ...
            @foreach($resourceUsage as $resource => $usage)
                ...
                <div class="limit-item {{ $isAtLimit ? 'limit-exceeded' : ($isNearLimit ? 'limit-warning' : '') }}">
                    <div class="d-flex align-items-center justify-content-between">
                        <div class="limit-info">
                            <i class="bi {{ $usage['icon'] }} text-muted"></i>
                            <span class="limit-name">{{ $usage['name'] }}</span>
                        </div>
                        <div class="limit-count">
                            <span class="current">{{ $usage['current'] }}</span>
                            <span class="separator">/</span>
                            <span class="total">{{ $usage['limit'] }}</span>
                        </div>
                    </div>
                    <div class="progress limit-progress">
                        <div class="progress-bar 
                            @if($isAtLimit) bg-danger
                            @elseif($isNearLimit) bg-warning
            ...
            <div class="sidebar-plan-info">
                ...
                <a href="{{ route('subscriptions.manage') }}" class="btn btn-sm btn-outline-primary w-100 mt-2">
