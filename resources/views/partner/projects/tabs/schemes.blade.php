...
<div id="schemes-tab-content">
    ...
    <div class="card mb-4" id="schemeFiltersCard">
        ...
        <div class="card-body">
            <form method="GET" action="{{ route('partner.projects.schemes', $project) }}" class="needs-validation" novalidate>
                <div class="row g-3">
                    <div class="col-6 col-md-3">
                        ...
                        <select class="form-select" id="schemeTypeFilter" name="scheme_type">
                            ...
                            @foreach($schemeTypeOptions as $value => $label)
                                <option value="{{ $value }}" {{ $filters['scheme_type'] == $value ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-3 d-flex align-items-end">
                        <div class="btn-group w-100" role="group">
                            <a href="{{ route('partner.projects.schemes', $project) }}" class="btn btn-outline-secondary" title="Сбросить все фильтры">
                    ...
                    <div class="col-6 col-md-4 col-lg-3">
                        <div class="card scheme-card h-100">
                            <div class="scheme-preview">
                                @if(in_array(strtolower(pathinfo($scheme->original_name, PATHINFO_EXTENSION)), ['jpg', 'jpeg', 'png', 'gif', 'webp', 'svg']))
                                    <img src="{{ asset('storage/' . $scheme->file_path) }}" alt="{{ $scheme->original_name }}" class="img-fluid">
                                @else
                                    <div class="file-icon">
                                        @switch(strtolower(pathinfo($scheme->original_name, PATHINFO_EXTENSION)))
                                            @case('pdf')
                                    ...
                                    <div class="btn-group-vertical" role="group">
                                        @if(in_array(strtolower(pathinfo($scheme->original_name, PATHINFO_EXTENSION)), ['jpg', 'jpeg', 'png', 'gif', 'webp', 'svg']))
                                            <button type="button" class="btn btn-sm btn-outline-light" data-bs-toggle="modal" data-bs-target="#viewSchemeModal" 
                                                    data-image="{{ asset('storage/' . $scheme->file_path) }}" data-title="{{ $scheme->original_name }}"
                                                    title="Просмотр">
                                                <i class="bi bi-eye"></i>
                                            </button>
                                        @endif
                                        <a href="{{ route('partner.projects.schemes.download', [$project, $scheme->id]) }}" 
                                           class="btn btn-sm btn-outline-light" title="Скачать">
                                            <i class="bi bi-download"></i>
                                        </a>
                                        @if(\App\Helpers\UserRoleHelper::canSeeActionButtons())
                                            <form method="POST" action="{{ route('partner.projects.schemes.delete', [$project, $scheme->id]) }}" 
                                                  style="display: inline;" 
                                                  onsubmit="return confirm('Вы уверены, что хотите удалить схему {{ $scheme->original_name }}?')">
                                                @csrf
                                                @method('DELETE')
                                ...
                                @if($scheme->scheme_type)
                                    <div class="scheme-type-badge">
                                        {{ $schemeTypeOptions[$scheme->scheme_type] ?? $scheme->scheme_type }}
                                    </div>
                                @endif
                            </div>
                            
                            <div class="card-body p-2">
                                <h6 class="card-title mb-1 text-truncate" title="{{ $scheme->original_name }}">
                                    {{ $scheme->original_name }}
                                </h6>
                                <div class="text-muted small">
                                    <div class="d-flex justify-content-between">
                                        <span>{{ number_format($scheme->file_size / 1024, 1) }} КБ</span>
                                        <span>{{ $scheme->created_at->format('d.m.Y') }}</span>
                                    </div>
                                    @if($scheme->room)
                                        <div class="mt-1">
                                            <i class="bi bi-geo-alt me-1"></i>{{ $roomOptions[$scheme->room] ?? $scheme->room }}
                                        </div>
                                    @endif
                                </div>
                            </div>
