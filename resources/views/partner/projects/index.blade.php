...
@section('styles')
<link rel="stylesheet" href="{{ asset('css/mobile-projects.css') }}">
@endsection
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-4 gap-3">
                ...
                <a href="{{ route('partner.projects.create') }}" class="btn btn-primary btn-lg d-md-none w-100">
                    <i class="bi bi-plus-circle me-2"></i>
                    Создать объект
                </a>
                <a href="{{ route('partner.projects.create') }}" class="btn btn-primary d-none d-md-inline-flex">
                    <i class="bi bi-plus-circle me-2"></i>
                    Создать объект
                </a>
                @endif
            ...
            <div class="card mb-4">
                <div class="card-body">
                    ...
                    <div class="collapse d-md-block" id="filtersCollapse">
                        <form method="GET" action="{{ route('partner.projects.index') }}" id="filterForm">
                            <input type="hidden" name="view_mode" value="{{ $viewMode ?? 'list' }}" id="viewModeInput">
                            <div class="row g-3">
                                ...
                                <div class="col-12 col-md-3">
                                    ...
                                    <input type="text" class="form-control form-control-lg d-md-none" id="phone" name="phone" 
                                           value="{{ request('phone') }}" placeholder="+7 (999) 123-45-67">
                                ...
                                <div class="col-6 col-md-2">
                                    ...
                                    <select class="form-select form-select-lg d-md-none" id="object_type" name="object_type">
                                        ...
                                        @foreach(\App\Models\Project::getObjectTypes() as $key => $label)
                                            <option value="{{ $key }}" {{ request('object_type') == $key ? 'selected' : '' }}>
                                                {{ $label }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-12 col-md-2 d-flex align-items-end">
                                    <div class="d-grid d-md-flex gap-2 w-100">
                                        ...
                                        <a href="{{ route('partner.projects.index') }}" class="btn btn-outline-secondary btn-lg d-md-none">
                                            <i class="bi bi-x-circle me-2"></i> Сбросить
                                        </a>
                                        <a href="{{ route('partner.projects.index') }}" class="btn btn-outline-secondary d-none d-md-inline-flex">
            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center flex-wrap">
                    <h5 class="mb-0 mb-2 mb-md-0">Список объектов ({{ $projects->total() }})</h5>
                    
                    <!-- Переключатель режимов - адаптивный -->
                    <div class="btn-group w-100 w-md-auto" role="group" aria-label="Режим отображения">
                        <input type="radio" class="btn-check" name="viewMode" id="listView" value="list" {{ ($viewMode ?? 'list') === 'list' ? 'checked' : '' }}>
                        ...
                        <input type="radio" class="btn-check" name="viewMode" id="cardView" value="cards" {{ ($viewMode ?? 'list') === 'cards' ? 'checked' : '' }}>
                ...
                <div class="card-body">
                                        ...
                                        <div class="card-body p-3">
                                            <div class="d-flex justify-content-between align-items-start mb-2">
                                                <h6 class="card-title mb-0 text-primary">
                                                    <i class="bi bi-building me-2"></i>
                                                    #{{ $project->id }} {{ $project->client_full_name }}
                                                </h6>
                                                @php
                                                ...
                                                @endphp
                                                <span class="badge {{ $statusClass }} fs-mobile-xs">
                                                    {{ \App\Models\Project::getStatuses()[$project->project_status] ?? $project->project_status }}
                                                </span>
                                            </div>
                                            ...
                                            <div class="row g-2 mb-3">
                                                <div class="col-6">
                                                    ...
                                                    <a href="tel:{{ $project->client_phone }}" class="text-decoration-none fw-bold">
                                                ...
                                                <div class="col-6">
                                                    ...
                                                    <span class="badge bg-info fs-mobile-xs">
                                                        {{ \App\Models\Project::getObjectTypes()[$project->object_type] ?? $project->object_type }}
                                                    </span>
                                                </div>
                                            </div>
                                            
                                            @if($project->object_full_address)
                                                <div class="mb-2">
                                            ...
                                            <div class="d-flex justify-content-between align-items-center mb-3">
                                                <div>
                                                    ...
                                                    @if($project->total_cost > 0)
                                                        <strong class="text-success">{{ number_format($project->total_cost, 0, ',', ' ') }} ₽</strong>
                                                    @else
                                            ...
                                            <div class="d-grid gap-2">
                                                ...
                                                <div class="btn-group w-100" role="group">
                                                    <a href="{{ route('partner.projects.edit', $project) }}" 
                                    ...
                                    <tbody>
                                        @foreach($projects as $project)
                                            <tr>
                                                ...
                                                <td>
                                                    <div>{{ $project->client_full_name }}</div>
                                                    @if($project->client_email)
                                                        <small class="text-muted">{{ $project->client_email }}</small>
                                                    @endif
                                                </td>
                                                <td>
                                                    <a href="tel:{{ $project->client_phone }}" class="text-decoration-none">
                                                        {{ $project->client_phone }}
                                                    </a>
                                                </td>
                                                <td>
                                                ...
                                                <td>
                                                    @if($project->object_full_address)
                                                        <small>{{ $project->object_full_address }}</small>
                                                    @else
                                                ...
                                                <td>
                                                    <div class="btn-group" role="group">
                                                        ...
                                                        <button class="btn btn-sm btn-outline-danger" 
                                                                onclick="confirmDelete({{ $project->id }})" title="Удалить">
                    ...
                    @else
                        <div class="text-center py-5">
                            ...
                            @if(\App\Helpers\UserRoleHelper::canManageProjects())
                            <a href="{{ route('partner.projects.create') }}" class="btn btn-primary btn-lg d-md-none">
                                <i class="bi bi-plus-circle me-2"></i>
                                Создать объект
                            </a>
                            <a href="{{ route('partner.projects.create') }}" class="btn btn-primary d-none d-md-inline-flex">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const savedViewMode = localStorage.getItem('projectsViewMode') || '{{ $viewMode ?? "list" }}';
