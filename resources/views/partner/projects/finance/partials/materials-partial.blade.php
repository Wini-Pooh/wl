            ...
            <tbody>
                @foreach($materials as $material)
                    <tr>
                        ...
                        <td>
                            <span class="badge bg-light text-dark">{{ number_format($material->quantity, 2) }}</span>
                        </td>
                        <td>
                            <span class="text-muted">{{ $material->unit }}</span>
                        </td>
                        <td>
                            <span class="text-success fw-semibold">{{ number_format($material->unit_price, 2) }} ₽</span>
                        </td>
                        <td>
                            <span class="badge bg-success text-white">{{ number_format($material->amount, 2) }} ₽</span>
                        </td>
                        <td>
                            <small class="text-info">
                                <i class="bi bi-calendar me-1"></i>{{ $material->created_at->format('d.m.Y') }}
                            </small>
                        </td>
                        <td class="text-end">
