            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="card mb-4">
                        <div class="card-header d-flex justify-content-between align-items-center filter-header-clickable" data-bs-toggle="collapse" data-bs-target="#filtersContent" aria-expanded="false" aria-controls="filtersContent" style="cursor: pointer;">
                            ...
                            <div class="d-flex align-items-center gap-2">
                                ...
                                @endphp
                                @if($activeFilters > 0)
                                    <span class="badge bg-primary">{{ $activeFilters }} активных</span>
                                @endif
                                <i class="fas fa-chevron-down" id="toggleFiltersIcon"></i>
                            </div>
                        </div>
                        <div class="card-body collapse" id="filtersContent" @if($activeFilters > 0) show @endif>
                            <form method="GET" action="{{ route('documents.index') }}" id="documentFilters">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="text" class="form-control" id="searchFilter" name="search" 
                                                   value="{{ request('search') }}" placeholder="Название, содержимое, получатель...">
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        ...
                                        <select class="form-select" id="typeFilter" name="document_type">
                                            ...
                                            <option value="contract" {{ request('document_type') == 'contract' ? 'selected' : '' }}>Договор</option>
                                            <option value="act" {{ request('document_type') == 'act' ? 'selected' : '' }}>Акт</option>
                                            <option value="invoice" {{ request('document_type') == 'invoice' ? 'selected' : '' }}>Счет</option>
                                            <option value="estimate" {{ request('document_type') == 'estimate' ? 'selected' : '' }}>Смета</option>
                                            <option value="technical" {{ request('document_type') == 'technical' ? 'selected' : '' }}>Техническая документация</option>
                                            <option value="other" {{ request('document_type') == 'other' ? 'selected' : '' }}>Прочее</option>
                                        </select>
                                    </div>
                                    <div class="col-md-3">
                                        ...
                                        <select class="form-select" id="statusFilter" name="status">
                                            ...
                                            <option value="received" {{ request('status') == 'received' ? 'selected' : '' }}>Получен</option>
                                        ...
                                        <select class="form-select" id="projectFilter" name="project_id">
                                            ...
                                            @foreach(\App\Models\Project::all() as $project)
                                                <option value="{{ $project->id }}" {{ request('project_id') == $project->id ? 'selected' : '' }}>
                                                    {{ $project->client_last_name }} {{ $project->client_first_name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                                ...
                                <div class="row g-3 mb-3">
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <select class="form-select" id="signatureFilter" name="signature_status">
                                            ...
                                            <option value="not_required" {{ request('signature_status') == 'not_required' ? 'selected' : '' }}>Не требуется</option>
                                            <option value="pending" {{ request('signature_status') == 'pending' ? 'selected' : '' }}>Ожидает подписи</option>
                                            <option value="signed" {{ request('signature_status') == 'signed' ? 'selected' : '' }}>Подписан</option>
                                            <option value="rejected" {{ request('signature_status') == 'rejected' ? 'selected' : '' }}>Отклонен</option>
                                        </select>
                                    </div>
                                    <div class="col-md-3">
                                ...
                                <div class="d-flex gap-2">
                                    ...
                                    <a href="{{ route('documents.index', ['tab' => $tab]) }}" class="btn btn-outline-secondary">
                    ...
                    <ul class="nav nav-tabs mb-4" id="documentTabs" role="tablist">
                        <li class="nav-item" role="presentation">
                            <button class="nav-link {{ $tab === 'received' ? 'active' : '' }}" 
                                ...
                                @if($tab === 'received' && isset($documents))
                                    <span class="badge bg-primary ms-1">{{ $documents->total() ?? 0 }}</span>
                                @endif
                            </button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link {{ $tab === 'created' ? 'active' : '' }}" 
                                ...
                                @if($tab === 'created' && isset($documents))
                                    <span class="badge bg-primary ms-1">{{ $documents->total() ?? 0 }}</span>
                                @endif
                            </button>
                        </li>
                    ...
                    <div class="tab-content" id="documentsTabContent">
                        ...
                        {{-- Контент вкладки шаблонов временно скрыт
            ...
            <div class="step-section mb-4" id="recipientStep">
                ...
                <div id="employeeSelection" class="recipient-section" style="display: none;">
                    ...
                    <select class="form-select" name="employee_id">
                        ...
                        @foreach(\App\Models\User::whereHas('roles', function($query) { $query->whereIn('name', ['partner', 'employee', 'foreman', 'estimator']); })->orWhereHas('defaultRole', function($query) { $query->whereIn('name', ['partner', 'employee', 'foreman', 'estimator']); })->get() as $user)
                ...
                <div id="clientSelection" class="recipient-section" style="display: none;">
                    ...
                    <select class="form-select" name="project_id" id="projectSelect">
                        ...
                        @foreach(\App\Models\Project::where('partner_id', auth()->id())->get() as $project)
                            <option value="{{ $project->id }}" 
                                    ...
                                    data-object-address="{{ $project->object_address }}">
                                {{ $project->client_last_name }} {{ $project->client_first_name }} - {{ $project->object_address }}
                            </option>
                        @endforeach
                    </select>
