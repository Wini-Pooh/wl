    ...
    <div class="col-12">
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <div class="d-flex align-items-center gap-2">
                            <span class="text-muted small">
                                Показано {{ $documents->count() }} из {{ $documents->total() }}
                            </span>
                        </div>
                    </div>
                </div>
            ...
            <div class="row g-2">
                @foreach($documents as $document)
                    <div class="col-6 col-sm-4 col-md-3 col-lg-3 col-xl-3 col-xxl-3">
                        <div class="card document-card h-100 shadow-sm" data-document-id="{{ $document->id }}">
                            ...
                            <div class="card-body p-2">
                                ...
                                <div class="mb-2 text-center">
                                    <span class="badge bg-secondary badge-sm" style="font-size: 0.6rem;">
                                        {{ Str::limit($document->type_name ?? 'Тип', 10) }}
                                    </span>
                                    @if($document->template)
                                        <small class="text-info d-block mt-1" style="font-size: 0.65rem;">
                                            <i class="fas fa-file-alt"></i> {{ Str::limit($document->template->name, 12) }}
                                        </small>
                                    @endif
                                </div>
                                ...
                                @if($document->project)
                                    <div class="mb-2 text-center">
                                        <small class="text-muted" style="font-size: 0.65rem;">
                                            ...
                                            {{ Str::limit($document->project->client_last_name . ' ' . $document->project->client_first_name, 12) }}
                                        </small>
                                    </div>
                                @endif
                                ...
                                <div class="text-muted text-center" style="font-size: 0.65rem;">
                                    ...
                                    <br>{{ $document->created_at->format('H:i') }}
                                    @if($document->expires_at)
                                        <div class="text-warning mt-1">
                                            <i class="fas fa-clock"></i> {{ $document->expires_at->format('d.m') }}
                                        </div>
                                    @endif
                                </div>
                            </div>
                                ...
                                <div class="btn-group w-100" role="group">
                                    ...
                                    @if($tab === 'created' && $document->status === 'draft')
                                        <button type="button" 
                                                ...
                                                data-document-id="{{ $document->id }}" title="Отправить">
                ...
                @if($documents->hasPages())
                    <nav aria-label="Page navigation">
                        <ul class="pagination">
                            ...
                            @if($start > 1)
                                <li class="page-item"><a class="page-link" href="#" data-page="1" data-tab="{{ $tab }}">1</a></li>
                                @if($start > 2)
                                    <li class="page-item disabled"><span class="page-link">...</span></li>
                                @endif
                            @endif
                            ...
                            @if($end < $documents->lastPage())
                                @if($end < $documents->lastPage() - 1)
                                    <li class="page-item disabled"><span class="page-link">...</span></li>
                                @endif
                                <li class="page-item"><a class="page-link" href="#" data-page="{{ $documents->lastPage() }}" data-tab="{{ $tab }}">{{ $documents->lastPage() }}</a></li>
                            @endif
        ...
        @else
            <div class="text-center py-5">
                ...
                {{-- Кнопка создания документа временно убрана
                @if($tab !== 'received')
...
window.bulkActionSignDocuments = function(documentIds) {
    const promises = documentIds.map(documentId => {
        return signDocumentRequest(documentId, '{{ auth()->user()->name }}', true);
    });
